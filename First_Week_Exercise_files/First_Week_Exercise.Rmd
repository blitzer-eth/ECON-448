...
global_medians <-
  ...
  group_by(year) %>%
  summarise(median_lifeExp = median(lifeExp), .groups = "drop")
...
df_cont_2007 <-
  gapminder %>%
  dplyr::filter(continent == "Asia", year == 2007)

ggplot(df_cont_2007, aes(x = gdpPercap, y = lifeExp)) +
  geom_point(alpha = 0.85) +
  scale_x_log10(labels = scales::label_comma()) +
  labs(
    title = "Life Expectancy vs GDP per Capita (Asia, 2007)",
...
ggplot(df_country, aes(x = gdpPercap, y = lifeExp)) +
  geom_point(alpha = 0.85) +
  scale_x_log10(labels = scales::label_comma()) +
  labs(
    title = paste0("Life Expectancy vs GDP per Capita (Taiwan, 1952–2007)"),
    x = "GDP per Capita (log scale)",
    y = "Life Expectancy (years)"
  ) +
  theme_minimal(base_size = 12)
...
A: I have used the R language in several classes, including Economics, Applied Math, and Informatics.
****
